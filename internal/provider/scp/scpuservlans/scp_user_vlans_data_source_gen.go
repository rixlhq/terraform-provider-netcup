		...
		Attributes: map[string]schema.Attribute{
			"scp_user_vlans": schema.SetNestedAttribute{
				NestedObject: schema.NestedAttributeObject{
					...
					CustomType: ScpUserVlansType{
						ObjectType: types.ObjectType{
							AttrTypes: ScpUserVlansValue{}.AttributeTypes(ctx),
						},
					},
				},
				Computed: true,
			...
			"server_id": schema.Int64Attribute{
				Optional:            true,
				Computed:            true,
				Description:         "Filter VLans of a user by server id",
				MarkdownDescription: "Filter VLans of a user by server id",
			},
...
type ScpUserVlansModel struct {
	ScpUserVlans types.Set   `tfsdk:"scp_user_vlans"`
	ServerId     types.Int64 `tfsdk:"server_id"`
	UserId       types.Int64 `tfsdk:"user_id"`
}

var _ basetypes.ObjectTypable = ScpUserVlansType{}

type ScpUserVlansType struct {
	basetypes.ObjectType
}

func (t ScpUserVlansType) Equal(o attr.Type) bool {
	other, ok := o.(ScpUserVlansType)
...
func (t ScpUserVlansType) String() string {
	return "ScpUserVlansType"
}

func (t ScpUserVlansType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	var diags diag.Diagnostics

	attributes := in.Attributes()

	bandwidthClassAttribute, ok := attributes["bandwidth_class"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`bandwidth_class is missing from object`)

		return nil, diags
	}

	bandwidthClassVal, ok := bandwidthClassAttribute.(basetypes.ObjectValue)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`bandwidth_class expected to be basetypes.ObjectValue, was: %T`, bandwidthClassAttribute))
	}
	...
	return ScpUserVlansValue{
		BandwidthClass: bandwidthClassVal,
		Name:           nameVal,
		Site:           siteVal,
		User:           userVal,
		VlanId:         vlanIdVal,
...
func NewScpUserVlansValueNull() ScpUserVlansValue {
	return ScpUserVlansValue{
		state: attr.ValueStateNull,
	}
}

func NewScpUserVlansValueUnknown() ScpUserVlansValue {
	return ScpUserVlansValue{
		state: attr.ValueStateUnknown,
	}
}

func NewScpUserVlansValue(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) (ScpUserVlansValue, diag.Diagnostics) {
	var diags diag.Diagnostics
	...
	for name, attributeType := range attributeTypes {
		...
		if !ok {
			diags.AddError(
				"Missing ScpUserVlansValue Attribute Value",
				"While creating a ScpUserVlansValue value, a missing attribute value was detected. "+
					"A ScpUserVlansValue must contain values for all attributes, even if null or unknown. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("ScpUserVlansValue Attribute Name (%s) Expected Type: %s", name, attributeType.String()),
			)
		...
		if !attributeType.Equal(attribute.Type(ctx)) {
			diags.AddError(
				"Invalid ScpUserVlansValue Attribute Type",
				"While creating a ScpUserVlansValue value, an invalid attribute value was detected. "+
					"A ScpUserVlansValue must use a matching attribute type for the value. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("ScpUserVlansValue Attribute Name (%s) Expected Type: %s\n", name, attributeType.String())+
					fmt.Sprintf("ScpUserVlansValue Attribute Name (%s) Given Type: %s", name, attribute.Type(ctx)),
			)
		}
	}
	...
	for name := range attributes {
		...
		if !ok {
			diags.AddError(
				"Extra ScpUserVlansValue Attribute Value",
				"While creating a ScpUserVlansValue value, an extra attribute value was detected. "+
					"A ScpUserVlansValue must not contain values beyond the expected attribute types. "+
					"This is always an issue with the provider and should be reported to the provider developers.\n\n"+
					fmt.Sprintf("Extra ScpUserVlansValue Attribute Name: %s", name),
			)
		}
	}

	if diags.HasError() {
		return NewScpUserVlansValueUnknown(), diags
	}
...
func NewScpUserVlansValueMust(attributeTypes map[string]attr.Type, attributes map[string]attr.Value) ScpUserVlansValue {
	object, diags := NewScpUserVlansValue(attributeTypes, attributes)
	...
	if diags.HasError() {
		...
		panic("NewScpUserVlansValueMust received error(s): " + strings.Join(diagsStrings, "\n"))
	}

	return object
}

func (t ScpUserVlansType) ValueFromTerraform(ctx context.Context, in tftypes.Value) (attr.Value, error) {
	if in.Type() == nil {
		return NewScpUserVlansValueNull(), nil
	}
	...
	if !in.IsKnown() {
		return NewScpUserVlansValueUnknown(), nil
	}
	...
	return NewScpUserVlansValueMust(ScpUserVlansValue{}.AttributeTypes(ctx), attributes), nil
}

func (t ScpUserVlansType) ValueType(ctx context.Context) attr.Value {
	return ScpUserVlansValue{}
}

var _ basetypes.ObjectValuable = ScpUserVlansValue{}

type ScpUserVlansValue struct {
	BandwidthClass basetypes.ObjectValue `tfsdk:"bandwidth_class"`
...
func (v ScpUserVlansValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	attrTypes := make(map[string]tftypes.Type, 5)

	var val tftypes.Value
	var err error

	attrTypes["bandwidth_class"] = basetypes.ObjectType{
		AttrTypes: BandwidthClassValue{}.AttributeTypes(ctx),
	}.TerraformType(ctx)
	...
	case attr.ValueStateKnown:
		...
		val, err = v.BandwidthClass.ToTerraformValue(ctx)

		if err != nil {
			return tftypes.NewValue(objectType, tftypes.UnknownValue), err
		}

		vals["bandwidth_class"] = val
...
func (v ScpUserVlansValue) IsNull() bool {
	return v.state == attr.ValueStateNull
}

func (v ScpUserVlansValue) IsUnknown() bool {
	return v.state == attr.ValueStateUnknown
}

func (v ScpUserVlansValue) String() string {
	return "ScpUserVlansValue"
}

func (v ScpUserVlansValue) ToObjectValue(ctx context.Context) (basetypes.ObjectValue, diag.Diagnostics) {
	var diags diag.Diagnostics

	var bandwidthClass basetypes.ObjectValue

	if v.BandwidthClass.IsNull() {
		bandwidthClass = types.ObjectNull(
			BandwidthClassValue{}.AttributeTypes(ctx),
		)
	}

	if v.BandwidthClass.IsUnknown() {
		bandwidthClass = types.ObjectUnknown(
			BandwidthClassValue{}.AttributeTypes(ctx),
		)
	}

	if !v.BandwidthClass.IsNull() && !v.BandwidthClass.IsUnknown() {
		bandwidthClass = types.ObjectValueMust(
			BandwidthClassValue{}.AttributeTypes(ctx),
			v.BandwidthClass.Attributes(),
		)
	}
	...
	attributeTypes := map[string]attr.Type{
		"bandwidth_class": basetypes.ObjectType{
			AttrTypes: BandwidthClassValue{}.AttributeTypes(ctx),
		},
...
func (v ScpUserVlansValue) Equal(o attr.Value) bool {
	other, ok := o.(ScpUserVlansValue)
	...
	if !v.BandwidthClass.Equal(other.BandwidthClass) {
		return false
	}
...
func (v ScpUserVlansValue) Type(ctx context.Context) attr.Type {
	return ScpUserVlansType{
		basetypes.ObjectType{
...
func (v ScpUserVlansValue) AttributeTypes(ctx context.Context) map[string]attr.Type {
