...
func (d *ScpUserIsoPartDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	path := fmt.Sprintf("/api/v1/users/{userId}/isos/%s/%s/parts/%s", data.Key.ValueString(), strconv.FormatInt(data.PartNumber.ValueInt64(), 10), data.UploadId.ValueString())
	...
	if arr, ok := jsonVal.([]interface{}); ok {
		jsonVal = map[string]interface{}{"scp_user_iso_part": arr}
	}
